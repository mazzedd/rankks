...
router.get('/gp/:slug/:year', async (req, res, next) => {
  try {
    ...
    const gp = await queryOne(`
      SELECT gp.id, gp.name, gp.slug, gp.circuit_country, gp.event_date, gp.is_sprint_weekend, gp.round_order, gp.scheduled_laps,
...
router.get('/session/:sessionId/results', async (req, res, next) => {
  try {
    ...
    const session = await queryOne(`
      SELECT s.id, s.session_type, gp.event_date, gp.f1_season_id
      FROM f1_sessions s
    ...
    if (!results.length) {
      const standings = await queryAll(`
        SELECT ds.driver_entity_id AS driver_id, d.canonical_name AS driver_name, d.slug AS driver_slug,
          d.image_url AS driver_image, d.birth_date, d.death_date,
          co.iso2 AS driver_country_iso2, co.name AS driver_country_name,
          t.id AS team_id, t.canonical_name AS team_name, t.image_url AS team_logo,
          ds.car_number
  ...
  try {
    ...
    const rows = await queryAll(`
      SELECT ds.position, ds.points, ds.wins, ds.p2, ds.p3, ds.sprint_wins, ds.car_number,
        d.id AS entity_id, d.canonical_name, d.slug, d.image_url AS logo_url, d.birth_date, d.death_date,
...
router.get('/drivers-all-time/:seasonId', async (req, res, next) => {
  try {
    ...
    // being dropped.
    const rows = await queryAll(`
      SELECT d.id AS entity_id, d.canonical_name, d.slug, d.image_url AS logo_url, d.birth_date, d.death_date,
        co.iso2 AS country_iso2, co.name AS country_name,
      ...
      LEFT JOIN (
        SELECT ds.driver_entity_id,
          ...
          COUNT(*) FILTER (WHERE ds.position = '1') AS championships,
    ...
    const rows = await queryAll(`
      ...
      LEFT JOIN (
        SELECT ts.team_entity_id,
          ...
          COUNT(*) FILTER (WHERE ts.position = '1') AS championships,
...
router.get('/races-all-time/:seasonId', async (req, res, next) => {
  try {
    ...
    const rows = await queryAll(`
      ...
      SELECT gm.slug, gm.latest_name, gm.first_year, gm.race_count,
        co.iso2 AS country_iso2, COALESCE(co.name, gm.latest_circuit_country) AS country_name,
        dw.canonical_name AS win_driver_name, dw.slug AS win_driver_slug, dw.death_date AS win_driver_death_date, tw.win_count,
        dp.canonical_name AS pole_driver_name, dp.slug AS pole_driver_slug, dp.death_date AS pole_driver_death_date, tp.pole_count,
        ds.canonical_name AS sprint_driver_name, ds.slug AS sprint_driver_slug, ds.death_date AS sprint_driver_death_date, tsp.sprint_count
      FROM gp_meta gm
      ...
      data: {
        ...
        races: rows.map(r => ({
          ...
          stats: {
            ...
            greater_wins: r.win_driver_name ? { driver_name: r.win_driver_name, driver_slug: r.win_driver_slug, death_date: r.win_driver_death_date, count: Number(r.win_count) } : null,
            greater_poles: r.pole_driver_name ? { driver_name: r.pole_driver_name, driver_slug: r.pole_driver_slug, death_date: r.pole_driver_death_date, count: Number(r.pole_count) } : null,
            greater_sprint_wins: r.sprint_driver_name ? { driver_name: r.sprint_driver_name, driver_slug: r.sprint_driver_slug, death_date: r.sprint_driver_death_date, count: Number(r.sprint_count) } : null,
          },
        })),
        count: rows.length,
      },
