...
const VALID_TYPES = ['sport', 'competition', 'club', 'athlete', 'media'];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const grouped = { sport: [], competition: [], club: [], athlete: [], media: [] };
...
async function getCapCheck(entity_type, userId) {
  ...
  if (entity_type === 'media') {
    const { count } = await queryOne(`
      SELECT count(*) FROM user_favourites WHERE user_id = $1 AND entity_type = 'media'
    `, [userId]);
