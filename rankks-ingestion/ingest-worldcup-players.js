// ingest-worldcup-players.js
// Adapted from ingest-ucl-players.js. Same entities/player_season_stats
// upsert logic — differences: national_team entities instead of club,
// dbYear = season directly (year_convention='start', no +1 offset),
// and hasRealGames narrowed to World Cup's two tab_groups only
// (group_stages, final_tour — no league_phase, that's UCL-only).
...
async function ingestPlayers(season, config, callApi) {
  ...
  let created = 0, updated = 0, statsUpserted = 0, teamSkipped = 0, qualifyingSkipped = 0;
  ...
  for (const item of allPlayers) {
    ...
    // ── 2. Upsert player_season_stats per national team ───────
    for (const s of stats) {
      if (!s.team?.id) continue;

      let team = await queryOne(
        `SELECT id FROM entities WHERE entity_type = 'national_team'
         AND canonical_name ILIKE $1`,
        [s.team.name]
      );
      if (!team) {
      ...
      if (!team) {
        console.log(`     ⚠️  National team not found: ${s.team.name} (player: ${p.name})`);
        teamSkipped++;
        continue;
      }

      // Skip teams with no real-tournament games this season — mirrors
      // ucl-players.js's qualifying-round exclusion. World Cup only has
      // group_stages + final_tour (no league_phase — that's UCL-only).
  ...
  console.log(`     ✅ Players: ${created} created, ${updated} updated, ${statsUpserted} stat rows upserted, ${teamSkipped} team-skipped, ${qualifyingSkipped} qualifying-only-skipped`);
}
