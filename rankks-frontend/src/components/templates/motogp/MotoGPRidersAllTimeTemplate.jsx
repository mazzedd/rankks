// templates/motogp/MotoGPRidersAllTimeTemplate.jsx
// All-Time > Riders — career cumulative stats "through <year>", same
// shape as F1DriversAllTimeTemplate.jsx minus Fastest Lap (no data source
// for it in MotoGP — see motogp.js's /standings/:seasonId/riders route
// header comment). Backed by /motogp/riders-all-time/:seasonId.
...
// first SPR session_type row is 2023) — the 'Sprint' entry is filtered
...
export default function MotoGPRidersAllTimeTemplate({ seasonId }) {
  ...
  useEffect(() => {
    if (!seasonId) return
    setLoading(true)
    api.getMotoGPRidersAllTime(seasonId)
      .then(setData)
      .catch(console.error)
      .finally(() => setLoading(false))
  }, [seasonId])

  // Admin-configured subtitle line — same fetch as F1DriversAllTimeTemplate.jsx,
  // scoped to competition 'motogp' (F1 and MotoGP share sport_slug
  // 'car-racing' but are separate competitions with separate subtitle rows).
  const [pageSubtitle, setPageSubtitle] = useState(null)
  useEffect(() => {
    if (!data?.year) return
    api.getSubtitle('car-racing', 'motogp', 'Totals', 'Rider Stats', data.year, data.season_status !== 'current')
      .then(d => setPageSubtitle(d?.subtitle || null))
      .catch(() => setPageSubtitle(null))
  }, [data?.year, data?.season_status])
                ...
                <tr key={d.entity_id} className="table-row" style={{ animationDelay: `${i * 0.03}s` }}>
                  ...
                  <td>
                    <div className="entity-cell">
                      <AthleteAvatar src={img} name={d.canonical_name} sport="moto-racing" gender="M" className="avatar" fallback="letter" />
                  ...
                  {showSprintCol && <td className={`stats-light ${sorted('sprint_wins')}`} style={td('center')}>{d.stats.sprint_wins}</td>}
...
function Empty() {
  return <div style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--text3)' }}>No rider data available.</div>
}
