...
export default function HomeTemplate({ pageTitle, competition, seasonId, year, competitionSlug }) {
  ...
  // NBA's Home (Mohamed 2026-08-16: "same template as FIFA World Cup")
  // reuses FootballHomeBlock as-is for the banner — it's already generic
  // (driven purely by competition.name/logo_url/short_code, nothing
  // football-specific) — and gets its own content component below since
  // NBA's four-stage-events shape doesn't match World Cup's one-season/
  // tab_group shape (see nba_home_template.jsx's own header comment).
  ...
  return (
    <>
      ...
      {(isQuadOrBiennial || isBasketball) && <FootballHomeBlock competition={competition} pageTitle={pageTitle} />}
      {isQuadOrBiennial ? (
        ...
        <NbaHomeTemplate competitionName={competition?.name} year={year} />
      ) : (
