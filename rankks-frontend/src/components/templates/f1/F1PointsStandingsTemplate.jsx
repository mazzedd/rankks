// templates/f1/F1PointsStandingsTemplate.jsx
// Columns: Pos | Driver (flag + country) | one column per round this
// season (3-letter code, chronological) | Total
// Backed by /f1/standings/:seasonId/points-by-race — every round of the
// season is a column, including any not yet raced (shows 0, never
// omitted). Default order is the API's own championship-position order
// (standings leaders first) — the "Sort by" dropdown lets a specific
// round's column be sorted instead, same sortRowsHighlight convention
// every other standings table on the site uses.
//
// Race/Sprint split — each round's cell shows the Race points on its own
// line, plus a small second line for Sprint points, but ONLY on rounds
// that actually have a sprint (round.has_sprint) — most F1 weekends
// don't, so most cells stay single-line.
//
// Slide arrows — the table can run to 24+ race columns, wider than the
// viewport. Rather than make the user hunt for the native scrollbar at
// the very bottom of a long driver list, two arrow buttons sit in the
// filter bar (same level as everything else, always visible) and scroll
// the table horizontally. Pinned to the far right, right before the
// drivers count, via a wrapper div with its own marginLeft:auto — the
// count's own CSS class also has marginLeft:auto, which would otherwise
// fight the wrapper's push and leave a gap, so it's cancelled locally.
//
// Sticky Rank/Driver columns — race columns scroll underneath; Rank and
// Driver stay pinned on the left so it's always clear which row is which.
...
// (shortGpLabel), not the raw stored name — the raw name's own word
// order varies by sport/era ("Australian Grand Prix" vs "Grand Prix of
// Thailand"), so truncating it directly gives inconsistent/wrong codes.
...
// Rank column is 48px (styles.pos) — Driver column sticks right after it.
...
export default function F1PointsStandingsTemplate({ seasonId, year }) {
  const [data, setData]       = useState(null)
  const [loading, setLoading] = useState(true)
  const [search, setSearch]   = useState('')
  const [sortGpId, setSortGpId] = useState('')
  // Podium pills — highlight every cell where that driver finished in
  // that exact position in the REGULAR RACE that round (never Sprint).
  ...
  useEffect(() => {
    if (!seasonId) return
    setLoading(true)
    api.getF1Standings(seasonId, 'points-by-race')
      .then(setData)
      .catch(console.error)
  ...
  useEffect(() => {
    if (!year) return
    api.getSubtitle('car-racing', 'formula-1-world-championship', 'Standings', 'Points Standings', year)
  ...
  // Sort by a specific round's race points, always descending — otherwise
  // keep the API's own championship-position order (default = leaders).
  if (sortGpId) {
    ...
    <div className={styles.wrap}>
      ...
      <div className="filter-bar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
          ...
          <span className="filter-total" style={{ marginLeft: 0 }}>{rows.length} drivers</span>
        </div>
      </div>
        ...
        <table className={`${styles.table} table-thead-border`} style={{ borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed' }}>
          <thead>
            <tr>
              ...
              <th className="table-label" style={{ ...th('left'), ...stickyDriver }}>Driver</th>
              {rounds.map(r => (
                <th
                  key={r.gp_id}
