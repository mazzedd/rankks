      ...
      <div className={styles.tableScroll}>
        ...
        <tbody>
          {rows.map((row, i) => {
            ...
            const age = calcAge(row.birth_date, data.season_end_date)
              ...
              <tr key={row.entity_id} className="table-row" style={{ animationDelay: `${i * 0.03}s` }}>
                ...
                <td>
                  <div className="entity-cell">
                    {img
                      ? <img src={img} alt={row.canonical_name} className="avatar" onError={e => { e.target.style.display = 'none' }} />
                      : <div className="avatar-placeholder">{(row.canonical_name || '?').charAt(0)}</div>
                    }
