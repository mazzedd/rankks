      ...
      <div className={styles.tableScroll}>
        ...
        <tbody>
          {races.map((race, i) => {
            ...
            const age = calcAge(race.birth_date, race.event_date)
            return (
              <tr key={race.race_id} className="table-row" style={{ animationDelay: `${i * 0.03}s` }}>
                ...
                <td style={{ textAlign: 'left' }}>
                  <div className="entity-cell">
                    {img
                      ? <img src={img} alt={race.driver_name} className="avatar" onError={e => { e.target.style.display = 'none' }} />
                      : <div className="avatar-placeholder">{(race.driver_name || '?').charAt(0)}</div>
                    }
